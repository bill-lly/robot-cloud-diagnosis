...
public class RobotInfoLookupFunction extends ProcessFunction<IncidentEvent, IncidentEvent> {

  private static final Logger LOG = LoggerFactory.getLogger(LookupProcessor.class);
  private final static String sql = "select product_id, alias, software_version, customer_code from robot where product_id = ?";
  ...
  public void processElement(IncidentEvent value, Context ctx, Collector<IncidentEvent> out) throws Exception {
    List<EntityBase> entities = lookupProcessor.getEntities(value.getKey(), resultSet -> {
      try {
        ...
        incidentEvent.setAlias(resultSet.getString(2));
        incidentEvent.setSoftwareVersion(resultSet.getString(3));
        incidentEvent.setCustomerCode(resultSet.getString(4));
    ...
    if (entities.isEmpty()) {
      ...
      for (EntityBase entity:entities) {
        ...
        value.setAlias(incidentEntity.getAlias());
        value.setSoftwareVersion(incidentEntity.getSoftwareVersion());
        value.setCustomerCode(incidentEntity.getCustomerCode());
        out.collect(value);
      }
    }
  }
